import{R as l,d as b,a as g,u as v,S as u,c as k,o as C,A as x}from"./sounds-DvokLOTz.js";import{T as p,B as M,M as i}from"./battle-E3W_S5g0.js";import{C as f,D as B}from"./dice-B1K2oSAH.js";const h={playerSheets:{},listeners:[],npcTemplates:[{name:"Orc",hp:30,type:p.ENEMY,color:"#dc2626"},{name:"Goblin",hp:10,type:p.ENEMY,color:"#16a34a"},{name:"Esqueleto",hp:20,type:p.ENEMY,color:"#e5e5e5"},{name:"Lobo",hp:15,type:p.ENEMY,color:"#6b7280"},{name:"Mago",hp:25,type:p.NPC,color:"#3b82f6"},{name:"Aldeão",hp:8,type:p.NPC,color:"#92400e"},{name:"Guarda",hp:20,type:p.NPC,color:"#1e40af"}],async initialize(){if(!l.isMaster){console.warn("MasterSystem: Apenas o mestre pode inicializar");return}await this.subscribeToPlayerSheets()},async subscribeToPlayerSheets(){const e=l.currentRoomCode;if(!e)return;const a=k(g,"sheets",e,"players"),t=C(a,n=>{n.forEach(s=>{var r;const o={id:s.id,...s.data()};s.id!==((r=x.currentUser)==null?void 0:r.uid)&&(this.playerSheets[s.id]=o)}),this.renderPlayerSheets()});this.listeners.push({ref:a,listener:t,type:"firestore"})},renderPlayerSheets(){const e=document.getElementById("masterPlayerSheets");if(!e)return;const a=Object.values(this.playerSheets);if(a.length===0){e.innerHTML='<p class="text-muted">Nenhuma ficha de jogador ainda</p>';return}e.innerHTML=a.map(t=>{var o,r;const n=Math.max(0,Math.min(100,(t.hp||0)/(t.hpMax||20)*100)),s=t.avatarUrl?`<img src="${t.avatarUrl}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`:`<span style="font-size:1.2rem;">${((r=(o=t.name)==null?void 0:o[0])==null?void 0:r.toUpperCase())||"?"}</span>`;return`
          ...
          ${n.map(o=>{var r;return`
            <label class="status-checkbox">
              <input type="checkbox" value="${o}" 
                ${(r=t.status)!=null&&r.includes(o)?"checked":""}
    ...
    `,document.body.appendChild(s)},async togglePlayerStatus(e,a,t){const n=l.currentRoomCode;if(!n)return;const s=b(g,"sheets",n,"players",e),o=this.playerSheets[e];if(!o)return;let r=[...o.status||[]];t&&!r.includes(a)?r.push(a):t||(r=r.filter(d=>d!==a)),await v(s,{status:r}),this.playerSheets[e]={...o,status:r},this.renderPlayerSheets()},openNpcModal(e){document.querySelectorAll(".modal-overlay").forEach(o=>o.remove());const a={hostile:"#dc2626",neutral:"#ca8a04",friendly:"#16a34a"},t={hostile:"Hostil",neutral:"Neutro",friendly:"Amigável"},n=Date.now(),s=document.createElement("div");s.className="modal-overlay",s.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,0.7);display:flex;align-items:center;justify-content:center;z-index:9999;",s.innerHTML=`
      <div style="background:#1a1208;border:1px solid #8a6a1a;border-radius:8px;padding:24px;min-width:300px;">
        ...
        <div style="display:flex;gap:8px;justify-content:flex-end;">
          ...
          <button onclick="MasterSystem.createNpcFromModal('${e}', '${n}')"
            style="background:${a[e]};border:none;color:white;padding:8px 16px;border-radius:4px;cursor:pointer;font-weight:bold;">
            Criar NPC
          </button>
        </div>
      </div>
    `,document.body.appendChild(s),document.getElementById(`npcModalName_${n}`).focus()},createNpcFromModal(e,a){var y,c,$,E;const t=((c=(y=document.getElementById(`npcModalName_${a}`))==null?void 0:y.value)==null?void 0:c.trim())||"NPC",n=parseInt(($=document.getElementById(`npcModalHp_${a}`))==null?void 0:$.value)||20,s=parseInt((E=document.getElementById(`npcModalHpMax_${a}`))==null?void 0:E.value)||20,o={hostile:"#dc2626",neutral:"#ca8a04",friendly:"#16a34a"},r={hostile:"enemy",neutral:"npc",friendly:"friendly"};document.querySelectorAll(".modal-overlay").forEach(S=>S.remove());const d={id:`npc_${Date.now()}`,name:t,hp:n,hpMax:s,status:"",type:r[e]||"npc",alignment:e,color:o[e]};this.npcCards||(this.npcCards={}),this.npcCards[d.id]=d,this.renderNpcCard(d)},renderNpcCard(e){var y;const a=document.getElementById("npcCardsContainer");if(!a)return;const t={hostile:"#dc2626",neutral:"#ca8a04",friendly:"#16a34a"},n={hostile:"Hostil",neutral:"Neutro",friendly:"Amigável"},s=["—","Caído","Sangrando","Paralizado","Atordoado","Envenenado","Assustado","Cego"],o=Math.max(0,Math.min(100,e.hp/e.hpMax*100)),r=t[e.alignment]||e.color||"#888";(y=document.getElementById(`npc-card-${e.id}`))==null||y.remove();const d=document.createElement("div");d.id=`npc-card-${e.id}`,d.draggable=!0,d.style.cssText=`
    ...
    `,d.innerHTML=`
      ...
      `:""}
    `,d.addEventListener("dragstart",c=>{c.dataTransfer.setData("npcCard",JSON.stringify(e)),c.dataTransfer.effectAllowed="copy"}),a.appendChild(d)},adjustNpcHP(e,a){var s;const t=(s=this.npcCards)==null?void 0:s[e];if(!t)return;t.hp=Math.max(0,Math.min(t.hpMax,t.hp+a)),this.npcCards[e]=t,this.renderNpcCard(t);const n=Object.values(i.tokens||{}).find(o=>o.npcCardId===e);n&&i.updateToken(n.id,{hp:t.hp})},setNpcHP(e,a){var r;const t=(r=this.npcCards)==null?void 0:r[e];if(!t)return;t.hp=Math.max(0,Math.min(t.hpMax,parseInt(a)||0)),this.npcCards[e]=t;const n=Math.max(0,Math.min(100,t.hp/t.hpMax*100)),s=document.getElementById(`npc-hpbar-${e}`);s&&(s.style.width=n+"%",s.style.background=n>50?"#16a34a":n>25?"#ca8a04":"#dc2626");const o=Object.values(i.tokens||{}).find(d=>d.npcCardId===e);o&&i.updateToken(o.id,{hp:t.hp})},setNpcStatus(e,a){var n;const t=(n=this.npcCards)==null?void 0:n[e];t&&(t.status=a,this.npcCards[e]=t,this.renderNpcCard(t))},removeNpc(e){var t,n;(t=this.npcCards)==null||delete t[e],(n=document.getElementById(`npc-card-${e}`))==null||n.remove();const a=Object.values(i.tokens||{}).find(s=>s.npcCardId===e);a&&i.removeToken(a.id)},async handleMapDrop(e){e.preventDefault();const a=e.dataTransfer.getData("npcCard");if(a){const n=JSON.parse(a);await i.addToken({name:n.name,type:n.type,hp:n.hp,hpMax:n.hpMax,color:n.color,npcCardId:n.id}),u.playNotification();return}const t=e.dataTransfer.getData("playerCard");if(t){const n=JSON.parse(t);await i.addToken({name:n.name,type:"player",hp:n.hp,hpMax:n.hpMax,avatarUrl:n.avatarUrl,color:"#c9a84c",ownerId:n.playerId}),u.playNotification();return}},async toggleBattleMode(){var n;const e=l.currentRoomCode;if(!e||!l.isMaster)return;const a=!((n=l.currentRoom)!=null&&n.battleMode),t=b(g,"rooms",e);return await v(t,{battleMode:a}),a?(await M.startBattle(),u.playBattleStart()):await M.endBattle(),a},async endSession(){if(!confirm("Tem certeza que deseja encerrar a sessão?"))return;const e=l.currentRoomCode;if(!e)return;const a=b(g,"rooms",e);await v(a,{status:"closed"}),await l.leaveRoom(e),window.location.href="index.html"},cleanup(){this.listeners.forEach(({ref:e,listener:a,type:t})=>{t==="firestore"&&a()}),this.listeners=[],this.playerSheets={}}};window.MasterSystem=h;const P=new URLSearchParams(window.location.search),m=P.get("code");async function N(){var e;try{if(console.log("[Master] Iniciando sistemas..."),await x.waitForInitialization(),!m||!x.isAuthenticated()){window.location.href="index.html";return}if(await l.joinRoom(m),!l.isMaster){console.warn("[Master] Acesso negado. Redirecionando para Lobby."),window.location.href="lobby.html?code="+m;return}document.getElementById("masterNick").textContent=x.currentNick,document.getElementById("roomName").textContent=((e=l.currentRoom)==null?void 0:e.name)||"Mesa",console.log("[Master] Inicializando Mapa..."),await i.initialize("mapContainer"),i.subscribeToMap();const a=document.querySelector(".map-wrapper");a&&(a.addEventListener("dragover",n=>n.preventDefault()),a.addEventListener("drop",n=>h.handleMapDrop(n)),console.log("[Master] Listeners de Drop no Mapa configurados.")),l.subscribeToRoom(m,{onRoomUpdate:n=>{document.getElementById("battleIndicator").classList.toggle("active",n.battleMode),n.battleMode&&(document.getElementById("toggleBattleBtn").textContent="⚔️ Desativar Batalha",document.getElementById("toggleBattleBtn").classList.remove("btn-success"),document.getElementById("toggleBattleBtn").classList.add("btn-danger"))},onPlayersUpdate:z}),console.log("[Master] Inicializando Chat e Batalha..."),f.subscribeToChat(m,I),f.subscribeToDice(m,T),await h.initialize(),await M.initialize(),window.DrawSystem&&window.DrawSystem.init();const t=document.getElementById("chatInput");t.disabled=!1,t.placeholder="Digite sua mensagem...",document.getElementById("chatSendBtn").disabled=!1,console.log("[Master] Sistemas prontos!")}catch(a){console.error("[Master] Erro crítico na inicialização:",a),alert("Erro ao carregar mesa. Verifique o console.")}}function z(e){const a=document.getElementById("playersList");a.innerHTML=e.map(t=>{var n,s;return`
      ...
      `).join("")||'<p class="text-muted" style="font-size: 0.85rem;">Nenhuma rolagem ainda</p>'}function L(){const e=document.getElementById("tokensList"),a=Object.values(i.tokens||{});e.innerHTML=a.map(t=>`
