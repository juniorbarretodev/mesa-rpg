import{R as l,d as h,a as p,u as v,S as d,c as M,o as S,A as g}from"./sounds-gK9Dals0.js";import{T as r,B as b,M as u}from"./battle-pCgxGZs_.js";import{C as y,D as w}from"./dice-CtbpK7_c.js";const c={playerSheets:{},listeners:[],npcTemplates:[{name:"Orc",hp:30,type:r.ENEMY,color:"#dc2626"},{name:"Goblin",hp:10,type:r.ENEMY,color:"#16a34a"},{name:"Esqueleto",hp:20,type:r.ENEMY,color:"#e5e5e5"},{name:"Lobo",hp:15,type:r.ENEMY,color:"#6b7280"},{name:"Mago",hp:25,type:r.NPC,color:"#3b82f6"},{name:"Aldeão",hp:8,type:r.NPC,color:"#92400e"},{name:"Guarda",hp:20,type:r.NPC,color:"#1e40af"}],async initialize(){if(!l.isMaster){console.warn("MasterSystem: Apenas o mestre pode inicializar");return}await this.subscribeToPlayerSheets(),await this.setupNPCPanel()},async subscribeToPlayerSheets(){const t=l.currentRoomCode;if(!t)return;const a=M(p,"sheets",t),e=S(a,s=>{s.forEach(n=>{var i;const o={id:n.id,...n.data()};n.id!==((i=g.currentUser)==null?void 0:i.uid)&&(this.playerSheets[n.id]=o)}),this.renderPlayerSheets()});this.listeners.push({ref:a,listener:e,type:"firestore"})},renderPlayerSheets(){const t=document.getElementById("masterPlayerSheets");if(!t)return;const a=Object.values(this.playerSheets);if(a.length===0){t.innerHTML='<p class="text-muted">Nenhuma ficha de jogador ainda</p>';return}t.innerHTML=a.map(e=>{var s,n,o,i;return`
    ...
    `,document.body.appendChild(n)},async togglePlayerStatus(t,a,e){const s=l.currentRoomCode;if(!s)return;const n=h(p,"sheets",s,t),o=this.playerSheets[t];if(!o)return;let i=[...o.status||[]];e&&!i.includes(a)?i.push(a):e||(i=i.filter(B=>B!==a)),await v(n,{status:i}),this.playerSheets[t]={...o,status:i},this.renderPlayerSheets()},setupNPCPanel(){const t=document.getElementById("npcList");t&&(t.innerHTML=this.npcTemplates.map(a=>`
    ...
    `).join(""),t.querySelectorAll(".npc-template").forEach(a=>{a.addEventListener("dragstart",e=>{e.dataTransfer.setData("npc",a.dataset.npc)})}))},async handleMapDrop(t){t.preventDefault();const a=t.dataTransfer.getData("npc");if(!a)return;const e=JSON.parse(a),s=document.querySelector(".map-wrapper");if(!s)return;const n=s.getBoundingClientRect();t.clientX-n.left,t.clientY-n.top,await u.addToken({name:e.name,type:e.type,hp:e.hp,hpMax:e.hp,color:e.color}),d.playNotification()},showAddNPCCustomModal(){const t=document.createElement("div");t.className="modal-overlay",t.innerHTML=`
      <div class="modal-content">
    ...
    `,document.body.appendChild(t)},async addCustomNPC(){const t=document.querySelector(".modal-overlay"),a=document.querySelector(".map-wrapper");if(!a)return;const e=a.getBoundingClientRect();e.width/2,e.height/2,await u.addToken({name:document.getElementById("customNpcName").value||"NPC",type:document.getElementById("customNpcType").value,hp:parseInt(document.getElementById("customNpcHp").value)||20,hpMax:parseInt(document.getElementById("customNpcHpMax").value)||20,color:document.getElementById("customNpcColor").value}),t==null||t.remove(),d.playNotification()},async toggleBattleMode(){var s;const t=l.currentRoomCode;if(!t||!l.isMaster)return;const a=!((s=l.currentRoom)!=null&&s.battleMode),e=h(p,"rooms",t);return await v(e,{battleMode:a}),a?(await b.startBattle(),d.playBattleStart()):await b.endBattle(),a},async endSession(){if(!confirm("Tem certeza que deseja encerrar a sessão?"))return;const t=l.currentRoomCode;if(!t)return;const a=h(p,"rooms",t);await v(a,{status:"closed"}),await l.leaveRoom(t),window.location.href="index.html"},cleanup(){this.listeners.forEach(({ref:t,listener:a,type:e})=>{e==="firestore"&&a()}),this.listeners=[],this.playerSheets={}}};window.MasterSystem=c;const P=new URLSearchParams(window.location.search),m=P.get("code");async function $(){var a;if(await g.waitForInitialization(),!m||!g.isAuthenticated()){window.location.href="index.html";return}if(await l.joinRoom(m),!l.isMaster){window.location.href="lobby.html?code="+m;return}document.getElementById("masterNick").textContent=g.currentNick,document.getElementById("roomName").textContent=((a=l.currentRoom)==null?void 0:a.name)||"Mesa",l.subscribeToRoom(m,{onRoomUpdate:e=>{document.getElementById("battleIndicator").classList.toggle("active",e.battleMode),e.battleMode&&(document.getElementById("toggleBattleBtn").textContent="⚔️ Desativar Batalha",document.getElementById("toggleBattleBtn").classList.remove("btn-success"),document.getElementById("toggleBattleBtn").classList.add("btn-danger"))},onPlayersUpdate:C}),y.subscribeToChat(m,N),y.subscribeToDice(m,k),c.setupNPCPanel(),await c.initialize(),await b.initialize();const t=document.getElementById("chatInput");t.disabled=!1,t.placeholder="Digite sua mensagem...",document.getElementById("chatSendBtn").disabled=!1}function C(t){const a=document.getElementById("playersList");a.innerHTML=t.map(e=>{var s,n;return`
      ...
      `).join("")||'<p class="text-muted" style="font-size: 0.85rem;">Nenhuma rolagem ainda</p>'}function I(){const t=document.getElementById("tokensList"),a=Object.values(u.tokens||{});t.innerHTML=a.map(e=>`
